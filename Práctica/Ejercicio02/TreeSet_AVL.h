...
protected:
   ...
   struct TreeNode {
      ...
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int v = 1, int t = 1) : elem(e), iz(i), dr(d), altura(alt), valor(v), tamanyo(t) {}
   };
...
public:
   ...
   T const& kesimo(int k) const {
       return kesimoP(k, raiz);
   }
protected:
   
    T const& kesimoP(int k, Link const& a) const {
        ...
        if (a == nullptr) {
            ...
            return kesimoP(k - a->valor, a->dr);
        }
        else if (a->valor > k) {
            return kesimoP(k, a->iz);
        }
   ...
   static Link copia(Link a) {
      ...
      else return new TreeNode(a->elem, copia(a->iz), copia(a->dr), a->altura, a->valor, a->tamanyo);
   }
   ...
   void rotaDer(Link & k2) {
      ...
      k2->valor = tamanyo(k2->iz) + 1;
      k2->tamanyo = tamanyo(k2->iz) + tamanyo(k2->dr) + 1;

      k1->altura = std::max(altura(k1->iz), altura(k1->dr)) + 1;
      k1->valor = tamanyo(k1->iz) + 1;
      k1->tamanyo = tamanyo(k1->iz) + tamanyo(k1->dr) + 1;
      k2 = k1;
   }
   ...
   void reequilibraIzq(Link & a) {
      if (altura(a->dr) - altura(a->iz) > 1) {
         if (altura(a->dr->iz) > altura(a->dr->dr))
          ...
          a->tamanyo = tamanyo(a->iz) + tamanyo(a->dr) + 1;
          a->valor = tamanyo(a->iz) + 1;
      }
   }
   ...
   /*
   static void reequilibrarNodos(Link& a) {

       if (a->dr == nullptr && a->iz == nullptr) {
           a->valor = 1;
           a->tamanyo = 1;
       }
       else if (a->dr == nullptr) {
           reequilibrarNodos(a->iz);
           a->valor = a->valor + 1;
           a->tamanyo = a->tamanyo + 1;
       }
       else if (a->iz == nullptr) {
           reequilibrarNodos(a->dr);
           a->tamanyo = a->tamanyo + 1;
       }
       else {
           reequilibrarNodos(a->iz);
           ...
           a->tamanyo = a->iz->tamanyo + a->dr->tamanyo +1;
           a->valor = a->iz->tamanyo + 1;
       }
   }
